...
echo $file = $args[0] > "%TEMP_PS%"
echo $targetDir = $args[1] >> "%TEMP_PS%"
echo $saverFile = $args[2] >> "%TEMP_PS%"
echo $filename = $args[3] >> "%TEMP_PS%"
echo $port = $args[4] >> "%TEMP_PS%"
...
echo     if ($line -match [regex]::Escape($saverFile)) { $exists = $true; break } >> "%TEMP_PS%"
echo } >> "%TEMP_PS%"
echo if ($exists) { Write-Host 'Instance already exists in scrnodes.bat'; exit 0 } >> "%TEMP_PS%"
echo $lastRemIndex = -1 >> "%TEMP_PS%"
echo for ($i = 0; $i -lt $content.Count; $i++) { >> "%TEMP_PS%"
echo     if ($content[$i] -match '^REM [0-9]') { $lastRemIndex = $i } >> "%TEMP_PS%"
echo } >> "%TEMP_PS%"
...
echo for ($i = 0; $i -lt $content.Count; $i++) { >> "%TEMP_PS%"
echo     if ($content[$i] -match ':wait_loop') { $waitLoopIndex = $i; break } >> "%TEMP_PS%"
echo } >> "%TEMP_PS%"
echo if ($waitLoopIndex -lt 0) { Write-Host ':wait_loop not found'; exit 1 } >> "%TEMP_PS%"
...
echo     if ($content[$i] -match 'echo .*:[0-9]') { $lastEchoIndex = $i } >> "%TEMP_PS%"
echo } >> "%TEMP_PS%"
...
powershell -ExecutionPolicy Bypass -File "%TEMP_PS%" "%SCRNODES_BAT%" "%TARGET_DIR%" "%SAVER_JS_FILE%" "%FILENAME%" "%PORT%"
del "%TEMP_PS%" >nul 2>&1
